...
let
  ...
  applyModule = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # + broker deployment-default modules — so `scooter-rebuild switch` picks up the
      # agent's authored modules WITHOUT needing a --module. The DEPLOYMENT's .scooter module
      ...
      # local /etc/scooter/modules/*.nix). Building base-only here would just rebuild
      # the running config for nothing — and worse, a sandbox with a placeholder
      # nixpkgs (tests, or a not-yet-provisioned pod) can't build at all. Exit idle
      # WITHOUT building. Base-only re-converge still runs whenever there IS something
      # authored to pick up.
      if [ -z "$module" ]; then
        local_mods=""
        [ -d /etc/scooter/modules ] && local_mods=$(find -L /etc/scooter/modules -maxdepth 1 -name '*.nix' -type f 2>/dev/null | head -1)
        if [ -z "$local_mods" ]; then
      ...
      if [ -n "$module" ]; then
        ...
        echo "scooter-apply-module: building toplevel (base config + local modules)..."
        module_expr=""
      fi
