...
# dev-env-scooter-module + Tier-2); this proves the CLI surface + the /etc/scooter file
# operations against the real dirs.
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    # EMPTY module is a no-op, not a failed build. A deployment whose .scooter dir has an
    # empty/whitespace module.nix must exit idle (0) WITHOUT trying to build it — an empty
    # file is not a valid Nix module, so the boot converge would fail on every start. This
    # machine has nixpkgs=/dev/null, so a real build would fail — proving the guard skips
    # the build entirely.
