...
test.describe("whole-UI consistency through a normal turn", () => {
  test("every surface stays mutually consistent across idle → running → replied", async ({ chat, page, request, baseURL }) => {
    // CLUSTER-HONEST BUDGET = the cold-start budget plus this test's own work. A ceiling
    // below RUN_START_MS can never spend it. Why: PR #543.
    test.setTimeout(RUN_START_MS + 90_000);
    await chat.open();
  ...
  test("consistency holds at EVERY step of an 8-turn conversation (not just at the end)", async ({ chat, page, request, baseURL }) => {
    // The cold-start budget plus 8 turns of real exec + a per-step snapshot each — every
    // fake-agent turn shells `echo <text>` on the full target. Why: PR #543.
...
test.describe("whole-UI consistency around the QUEUE", () => {
  test("queueing keeps thread, queue, badge, run-state and composer mutually consistent", async ({ chat, page }) => {
    // The cold-start budget plus two sendWhileRunning retry loops and three whole-UI
    // snapshots. Why: PR #543.
    test.setTimeout(RUN_START_MS + 90_000);
    await chat.open();
  ...
  test("the queue DRAINS into the thread with counts conserved (nothing lost, nothing duplicated)", async ({ chat, page, request, baseURL }) => {
    // The cold-start budget plus the 60s drain this test waits on — a ceiling equal to the
    // wait dies at the same moment the wait would have. Why: PR #543.
  ...
  test("a reload mid-queue preserves EVERY surface, not just the queue rows", async ({ chat, page, request, baseURL }) => {
    // The cold-start budget plus the reload and its re-derive poll. Why: PR #543.
    test.setTimeout(RUN_START_MS + 90_000);
    await chat.open();
