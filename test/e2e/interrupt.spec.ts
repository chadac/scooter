 ...
 * renders them as inline buttons (the InterruptPanel) and resumes the run with
...
test.describe("agent option dropdown (interrupt)", () => {
  test("the agent presents options; picking one resumes the run", async ({ chat, page }) => {
    ...
    // The interrupt panel appears with the three options.
    ...
    // The panel goes away once the request is answered.
