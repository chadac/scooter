...
async def test_mention_twin_events_dispatch_exactly_once():
    """THE bug: an @mention arrives as BOTH an app_mention AND a message event
    (different event_ids, SAME message ts). The raw event text is `<@UBOT>` — the
    `@scooter` pattern doesn't match it — so text-skipping alone let the message
    twin through. The channel:ts dedup in _handle_event must drop whichever event
    loses the race, so the agent is prompted EXACTLY ONCE."""
    import asyncio

    slack_h._SEEN_MESSAGE_TS.clear()
    dispatched: list[str] = []
    with (
        patch.object(slack_h, "db") as db,
        patch.object(slack_h, "send_message", new=AsyncMock(return_value=True)) as send,
        ...
        db.lookup_conversation = AsyncMock(return_value="conv-1")
        db.get_conversation_for_resource = AsyncMock(return_value="conv-1")
        send.side_effect = lambda conv, msg, **kw: dispatched.append(msg) or True

        base = {"user": "U1", "channel": "C1", "ts": "1.0", "thread_ts": "1.0",
                "text": "<@UBOT> please review the PR"}
        # Both twin events for the SAME message (raw <@UBOT> encoding).
        await slack_h._handle_event({**base, "type": "app_mention"})
        await slack_h._handle_event({**base, "type": "message"})
        await asyncio.sleep(0)

    assert len(dispatched) == 1, f"expected ONE dispatch, got {len(dispatched)}: {dispatched}"


async def test_message_handler_skips_a_RAW_encoded_mention():
    """_handle_thread_message must skip a message whose text is the RAW `<@BOTID>`
    encoding (not the literal `@scooter`) — the form Slack actually sends. Without
    this, a message-event mention forwarded as a plain (non-priority) thread msg."""
    slack_h._SEEN_MESSAGE_TS.clear()
    with (
        patch.object(slack_h, "db") as db,
        patch.object(slack_h, "send_message", new=AsyncMock(return_value=True)) as send,
        ...
        await slack_h._handle_thread_message({
            "type": "message", "thread_ts": "1.0", "channel": "C1", "user": "U1",
            "text": "<@UBOT> please take another look",  # RAW encoding, not @scooter
        })
        send.assert_not_called()  # skipped — app_mention owns it
