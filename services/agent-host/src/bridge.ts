...
export interface SessionBridge {
   ...
   *  runs). Also emits a clearing QUEUE_UPDATED so the durable log's last word is
   *  "empty" (else a reattaching UI folds a phantom queued row nothing will drain).
   *
   *  `requeue`: whether the caller will RE-ENQUEUE the drained items on revive
   *  (suspend does; a plain teardown/end does not). It decides how each drained
   *  item's still-awaited prompt() promise settles:
   *   - requeue=false (default): REJECT — the message will not run, so its caller
   *     must see the failure.
   *   - requeue=true: RESOLVE — the message WILL run after revive, so rejecting it
   *     would surface a spurious RUN_ERROR to the client for a turn that in fact
   *     runs (the pod-cap eviction / suspend-mid-queue false failure). See PR. */
  drainQueue(opts?: { requeue?: boolean }): Array<{ text: string; priority: number }>;
...
export function createSessionBridge(deps: BridgeDeps): SessionBridge {
  ...
  const self: SessionBridge = {
    ...
    drainQueue(opts) {
      ...
      emitQueueSnapshot();
      // Settle only the items that never STARTED. An in-flight item is a run that is
      // already executing: its promise settles through the pump's own resolve/reject
      // when runPrompt returns (or throws as the pod goes away). Settling it here
      ...
      // terminates) would report a failure for a message that in fact ran and answered.
      //
      // When the caller will RE-ENQUEUE on revive (suspend), a waiting item is NOT
      // lost — it runs after resume — so REJECTING it surfaces a spurious RUN_ERROR
      // to the client for a turn that in fact runs (the pod-cap-eviction false
      // failure). RESOLVE it instead; the reply arrives on the integrity stream when
      // the revived run completes. Only a true teardown (requeue=false) rejects.
      for (const item of waiting) {
        if (opts?.requeue) item.resolve(item.id);
        else item.reject(new Error("the conversation was suspended before this queued message could run"));
      }
