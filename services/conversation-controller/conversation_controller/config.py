...
class Config:
    ...
    # Orphaned-Sandbox reaper: destroy Sandboxes with no owning Conversation. On by default.
    # TWO windows, because there are two different situations (see reconcile.find_orphans):
    # confirm = we watched the Conversation exist and go, so only a create-race needs riding
    # out; grace = we have never seen the Sandbox referenced, and cannot tell garbage from a
    # CR that has not registered spec.sandboxRef yet. One number for both is what left ~15
    # dead pods on the e2e-full node and exhausted it (#495).
    # See todo/docs/ORPHANED_SANDBOX_REAPER.md.
    reap_orphans: bool = True
    orphan_grace_seconds: float = 180.0
    orphan_confirm_seconds: float = 15.0
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            orphan_confirm_seconds=float(os.environ.get("ORPHAN_CONFIRM_SECONDS", "15")),
